...
const form = useSanctumForm("post", "/api/partner_register", {
  store_name: "",
  email: "",
  password: "",
    ...
    <div class="w-full max-w-md p-8 space-y-3 bg-white shadow-lg rounded-xl">
      ...
      <form @submit.prevent="registerUser">
        <div class="flex flex-col space-y-1">
          <label for="store_name" class="text-sm font-medium">Name</label>
          <input
            id="store_name"
            type="text"
            v-model="form.store_name"
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
            :class="{ 'border-red-600': form.invalid('store_name') }"
            @input="form.forgetError('store_name')"
          />
          <span v-if="form.invalid('store_name')" class="text-sm text-red-600">
